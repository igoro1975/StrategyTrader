...
namespace StrategyTrader
{
    ...
    {
        public RequestsClient Client { get; }
                 ...
                 || key == "UnrealizedPnL") && currency == "USD")
            {
                if (key == "NetLiquidation" || key == "UnrealizedPnL")
                {
                    ...
                    if (key == "NetLiquidation")
                    {
                        ...
                        Client.PushOrdersInfo(new Equity() { AccountID =AccountID, Value = Equity, UpdateTime = DateTime.Now }, PushMessageType.EquityUpdatePush);
                    }
                }
                Client.PushOrdersInfo(new AccountSummaryUpdate(AccountID, key, Convert.ToDouble(value, provider).ToString(CultureInfo.InvariantCulture)),
